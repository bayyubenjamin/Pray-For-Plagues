...
export default function Navbar() {
  ...
  return (
    <>
      <header className="hidden md:block fixed top-0 left-0 w-full z-50 bg-[#050705] border-b border-green/30">
      ...
      <header className="md:hidden fixed top-0 left-0 w-full z-50 bg-[#050705] border-b border-green/20 px-4 h-14 flex items-center justify-between">
        <Link href="/" className="hover:opacity-80 transition">
          <Image
            src="/images/logo.png"
            alt="Logo"
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 bg-[#050705] border-t border-green/30 px-4 py-2 flex items-center justify-around font-tech text-[10px] tracking-wider">
        ...
        <Link href="/profile" className={`flex flex-col items-center transition group py-1 ${isActive('/profile') ? 'text-green font-bold' : 'text-gray-400 hover:text-green'}`}>
          <svg className={`w-5 h-5 transition ${isActive('/profile') ? 'text-green drop-shadow-[0_0_8px_rgba(74,222,128,0.8)]' : 'group-hover:drop-shadow-[0_0_8px_rgba(74,222,128,0.8)]'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
