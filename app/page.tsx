...
export default function Home() {
  const previewNFTs = MOCK_NFTS.filter((n) =>
    (["MYTHIC", "LEGENDARY", "EPIC", "RARE"] as const).includes(n.rarity as "MYTHIC" | "LEGENDARY" | "EPIC" | "RARE")
  ).slice(0, 4);
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <section className="w-full min-h-screen flex flex-col items-center justify-center px-4 relative overflow-hidden">
        <ScrollReveal className="text-center max-w-4xl mx-auto z-20 flex flex-col items-center gap-4 sm:gap-6 mt-16 sm:mt-0">
          ...
          <p className="font-tech text-[10px] sm:text-xs tracking-[0.2em] text-gray uppercase">
            Your vial is empty. Your dose is not.
          </p>
      ...
      <section className="w-full max-w-6xl mx-auto px-4 py-16">
        <ScrollReveal>
          <SectionTitle title="OUTBREAK PROTOCOL" subtitle="FIVE LOCKS. ONE LIVE." />
        </ScrollReveal>
        <ScrollReveal>
          <ProtocolRoadmap />
        </ScrollReveal>
      ...
      <section className="w-full max-w-6xl mx-auto px-4 py-16">
        <ScrollReveal>
          <SectionTitle title="ANTIDOTE STRAINS" subtitle="2,000 VIALS · FIVE RARITIES" />
        </ScrollReveal>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
          {previewNFTs.map((nft, index) => (
            <ScrollReveal key={nft.id} delay={index * 100}>
              <AntidoteCard nft={nft} compact briefing />
            </ScrollReveal>
          ))}
        </div>
        <div className="text-center mt-8">
          <Link href="/vault" className="font-tech text-xs tracking-widest text-green hover:text-green-bright">
